...
static int clicmp(struct client *a, struct client *b) {
  if (a->pid == b->pid)
    return (0);
...
void client_init(uint winsz) {
  bwstat_init(winsz);

  SPLAY_INIT(&clients);
}

int client_register(struct client *cli) {
...
int client_configure(struct client *cli) {
...
void client_unregister(struct client *cli) {
...
void client_getinfo(struct client *cli, uint sendlim, uint recvlim) {
...
void client_delay(struct client *cli, short which, size_t len, uint lim) {
  struct timeval *tv;

  if ((tv = bwstat_getdelay(cli->stat, &len, lim, which)) != NULL) {
    warnxv(4, "Delay %d (%s/%s) by: %d bytes in %dsec%dusec START", cli->pid,
           cli->argv0, cli->uname, len, tv->tv_sec, tv->tv_usec);
    cli->delaytv = *tv;
    cli->delaylen = len;
...
void client_getdelay(struct client *cli, short which, size_t len, uint lim) {
  struct timeval *tv;
  struct msg msg;
  struct msg_delayinfo *delayinfo = &msg.data.delayinfo;
...
static void client_delaycb(int fd, short which, void *arg) {
  struct client *cli = arg;
  struct msg msg;
  struct msg_delayinfo *delayinfo = &msg.data.delayinfo;

  warnxv(4, "Delay %d (%s/%s) by %dsec%dusec END", cli->pid, cli->argv0,
         cli->uname, cli->delaytv.tv_sec, cli->delaytv.tv_usec);
...
void client_update(struct client *cli, short which, size_t len) {
  struct bwstat_data *bsd = &cli->stat->data[which];

  warnxv(4, "Statistics (%s) for %d (%s/%s):",
         which == TRICKLE_SEND ? "SEND" : "RECV", cli->pid, cli->argv0,
         cli->uname);
  ...
  warnxv(4, "\tavg: %d.%d KB/s; win: %d.%d KB/s", bsd->rate / 1024,
         (bsd->rate % 1024) * 100 / 1024, bsd->winrate / 1024,
         (bsd->winrate % 1024) * 100 / 1024);
}
...
void client_printrates(void) {
  ...
  warnxv(0,
         ...
         bsd->rate / 1024, (bsd->rate % 1024) * 100 / 1024, bsd->winrate / 1024,
         (bsd->winrate % 1024) * 100 / 1024);
...
int client_sendmsg(struct client *cli, struct msg *msg) {
  u_char buf[2048];
  ...
  if (atomicio(write, cli->s, &xbuflen, sizeof(xbuflen)) != sizeof(xbuflen))
    return (-1);
...
int client_recvmsg(struct client *cli, struct msg *msg) {
  u_char buf[2048];
  ...
  if (atomicio(read, cli->s, &xbuflen, sizeof(xbuflen)) != sizeof(xbuflen))
    return (-1);
