...
static int xdr_msg_conf(XDR *xdrs, struct msg_conf *conf) {
...
static int xdr_msg_delay(XDR *xdrs, struct msg_delay *delay) {
...
static int xdr_msg_update(XDR *xdrs, struct msg_update *update) {
...
static int xdr_msg_delayinfo(XDR *xdrs, struct msg_delayinfo *delayinfo) {
...
static int xdr_msg_getinfo(XDR *xdrs, struct msg_getinfo *getinfo) {
...
static bool_t _xdr_void(void) { return (TRUE); }

static int xdr_msg(XDR *xdrs, struct msg *msg) {
  X(xdr_short(xdrs, &msg->status));
  X(xdr_union(xdrs, (int *)&msg->type, (char *)&msg->data, xdr_msg_discrim,
              (xdrproc_t)_xdr_void));

  return (TRUE);
}

int msg2xdr(struct msg *msg, u_char *buf, uint32_t *buflen) {
  XDR xdrs;
...
int xdr2msg(struct msg *msg, u_char *buf, uint32_t buflen) {
  XDR xdrs;
  int ret = 0;
