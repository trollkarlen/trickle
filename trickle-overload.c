...
static int verbose;
static uint lim[2][7 * 24 * DIVS_PER_HOUR]; // 7 days in week * 24 hours in day
                                            // * divs in hour
static char *argv0;
...
DECLARE(recvfrom, ssize_t,
        (int, void *, size_t, int, struct sockaddr *, Psocklen_t));
#else
DECLARE(recvfrom, ssize_t,
...
/*
 * Because, select(),poll(), read(), write() and many other overloaded functions
...
static void trickle_lock(sigset_t *oset) {
   ...
   * if more than 1 thread enter trickle-overload while initialization is
   * performed they must wait until initialization is completed before
   * continuing. Hence the only place that I have found we could load
   * pthread_sigmask() is here.
   */
...
static void trickle_init(void) {
  void *dh;
  char *winszstr, *verbosestr, *recvlimstr, *sendlimstr, *sockname, *tsmoothstr,
      *lsmoothstr;
  /* 	    *latencystr; */
...
#pragma GCC diagnostic pop
  ...
  */
  ...
  trickled_configure(sockname, libc_socket, libc_read, libc_write, libc_close,
                     argv0);
...
int socket(int domain, int type, int protocol) {
...
#ifdef DEBUG
  safe_printv(0, "[DEBUG] socket(%d, %d, %d) = %d", domain, type, protocol,
              sock);
#endif /* DEBUG */
  ...
  if (sock != -1 && (domain == AF_INET || domain == AF_INET6) &&
      type & SOCK_STREAM) {
...
static struct delay *select_delay(struct delayhead *dhead, struct sockdesc *sd,
                                  short which) {
  ssize_t len = -1;
  struct timeval *delaytv;
  struct delay *d, *_d;
  ...
  safe_printv(3,
              "[trickle] Delaying socket (%s) %d "
              "by %ld seconds %ld microseconds",
              which == 0 ? "write" : "read", sd->sock, delaytv->tv_sec,
              delaytv->tv_usec);
...
static struct delay *select_shift(struct delayhead *dhead,
  ...
  if (*delaytv != NULL && ((*delaytv)->tv_sec < 0 || (*delaytv)->tv_usec < 0))
    timerclear(*delaytv);
...
int select(int nfds, fd_set *rfds, fd_set *wfds, fd_set *efds,
  ...
  if ((fds = fdsets[which]) != NULL && FD_ISSET(sd->sock, fds) &&
...
#define POLL_RDMASK                                                            \
  (POLLIN | /* POLLNORM | */ POLLPRI | POLLRDNORM | POLLRDBAND)

#if defined(__linux__) || (defined(__svr4__) && defined(__sun__)) ||           \
    defined(__OpenBSD__)
int poll(struct pollfd *fds, nfds_t nfds, int __timeout)
#elif defined(__FreeBSD__)
int poll(struct pollfd *fds, unsigned int nfds, int __timeout)
#else
int poll(struct pollfd *fds, int nfds, int __timeout)
#endif /* __linux__ */
{
  struct pollfd *pfd;
  ...
  struct sockdesc *sd;
  struct delay *d, *_d;
  struct timeval inittv, curtv, _timeout, *timeout = NULL, *delaytv, *polltv,
                                          difftv;
  struct delayhead dhead;
  LOCK_VAR;
...
again:
  ...
  if (polltv != NULL)
    polltimeout = polltv->tv_sec * 1000 + polltv->tv_usec / 100;
  else
    polltimeout = INFTIM;
...
ssize_t recv(int sock, void *buf, size_t len, int flags) {
  ...
  if (!(eagain = delay(sock, &xlen, TRICKLE_RECV) == TRICKLE_WOULDBLOCK)) {
    ...
    TRICKLE_LOCK;
#ifdef DEBUG
    safe_printv(0, "[DEBUG] recv(%d, *, %d, %d) = %d", sock, len, flags, ret);
  } else {
...
#endif /* __sun__ */
{
  ...
  if (!(eagain = delay(sock, &xlen, TRICKLE_RECV) == TRICKLE_WOULDBLOCK)) {
    ...
    TRICKLE_LOCK;
#ifdef DEBUG
    safe_printv(0, "[DEBUG] recvfrom(%d, *, %d, %d) = %d", sock, len, flags,
                ret);
  } else {
    safe_printv(0, "[DEBUG] delaying recvfrom(%d, *, %d, %d)", sock, len,
                flags);
#endif /* DEBUG */
  }
...
ssize_t writev(int fd, const struct iovec *iov, int iovcnt) {
  ...
  if (!(eagain = delay(fd, &len, TRICKLE_SEND) == TRICKLE_WOULDBLOCK)) {
    ...
    TRICKLE_LOCK;
#ifdef DEBUG
    safe_printv(0, "[DEBUG] writev(%d, *, %d) = %d", fd, iovcnt, ret);
  } else {
...
ssize_t send(int sock, const void *buf, size_t len, int flags) {
  ...
  if (!(eagain = delay(sock, &xlen, TRICKLE_SEND) == TRICKLE_WOULDBLOCK)) {
    ...
    TRICKLE_LOCK;
#ifdef DEBUG
    safe_printv(0, "[DEBUG] send(%d, *, %d, %d) = %d", sock, len, flags, ret);
  } else {
    safe_printv(0, "[DEBUG] delaying send(%d, *, %d, %d)", sock, len, flags);
#endif /* DEBUG */
  }
...
static int delay(int sock, ssize_t *len, short which) {
  struct sockdesc *sd;
  struct timeval *tv;
  ...
  if ((tv = getdelay(sd, len, which)) != NULL) {
    ...
    safe_printv(2, "[trickle] Delaying %lds%ldus", tv->tv_sec, tv->tv_usec);
...
static struct timeval *getdelay(struct sockdesc *sd, ssize_t *len,
                                short which) {
  struct timeval *xtv;
...
static void update(int sock, ssize_t len, short which) {
  struct sockdesc *sd;
...
void safe_printv(int level, const char *fmt, ...) {
  va_list ap;
  char str[1024];
  int n;
