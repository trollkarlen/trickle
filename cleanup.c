...
struct cleanup *cleanup_new(void) {
  struct cleanup *clup;
...
struct cleanup *cleanup_free(struct cleanup *clup) {
  struct cleanupq *cq;
...
int cleanup_add(struct cleanup *clup, void (*f)(void *), void *handler) {
  struct cleanupq *cq;
...
int cleanup_remove(struct cleanup *clup, void (*f)(void *), void *handler) {
...
void cleanup_cleanup(struct cleanup *clup) {
  struct cleanupq *cq;
...
void cleanupcb_close(void *_fd) {
  int fd = *(int *)_fd;

  close(fd);
}

void cleanupcb_unlink(void *_fname) {
