...
void trickled_configure(char *xsockname, int (*xlibc_socket)(int, int, int),
                        ssize_t (*xlibc_read)(int, void *, size_t),
                        ssize_t (*xlibc_write)(int, const void *, size_t),
                        int (*xlibc_close)(int), char *xargv0) {
  sockname = xsockname;
...
void trickled_open(int *xtrickled) {
  struct msg msg;
...
void trickled_close(int *xtrickled) {
  (*libc_close)(trickled_sock);
  *xtrickled = 0;
  trickled_sock = -1;
}

void trickled_ctl_open(int *xtrickled) {
  struct msg msg;
...
static void _trickled_open(struct msg *msg, int *xtrickled) {
  int s;
...
int trickled_sendmsg(struct msg *msg) {
  u_char buf[2048];
...
int trickled_recvmsg(struct msg *msg) {
  u_char buf[2048];
...
int trickled_update(short dir, size_t len) {
  struct msg msg;
...
int trickled_delay(short dir, size_t *len) {
  struct msg msg;
...
struct timeval *trickled_getdelay(short dir, size_t *len) {
  struct msg msg;
  struct msg_delay *delay = &msg.data.delay;
  struct msg_delayinfo *delayinfo = &msg.data.delayinfo;
...
int trickled_getinfo(uint32_t *uplim, uint32_t *uprate, uint32_t *downlim,
                     uint32_t *downrate) {
  struct msg msg;
